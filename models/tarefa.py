...
class Tarefa:
    ...
    excluir e atualizar tarefas em um banco de dados usando a classe `Database`.
    """
    ...
    def salvar_tarefa(self: Self) -> None:
        with Database() as db:
            query: str = """
            INSERT INTO tarefas (titulo_tarefa, 
            data_conclusao,
            concluida)
            VALUES (?,?,?);
            """
    ...
    def obter_tarefas(cls) -> list[Self]:
        with Database() as db:
            query: str = (
            "SELECT titulo_tarefa, data_conclusao, id, concluida FROM tarefas;"
            )
